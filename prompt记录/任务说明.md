...
1.  一定导出全部的联系人和群聊列表，方便据此判断，有哪些内容需要导出
2.  支持只导出指定的联系人或群聊，可以使用uid和type进行指定。若不指定，默认导出全部
3.  导出的每个联系人/群聊内容为一个文件夹，文件夹内为聊天内容
4.  聊天内容按以下规则输出
    1.  若总量小于10万条，则全部输出，文件名为`聊天开始年月-聊天结束年月`
    2.  若总量大于10万条，分文件输出，按月进行累加。不足10万条继续输出，超过10万条则新开一个文件。若单月内容超过10万条，则按月拆分，文件名额外添加part.1, part.2后缀。整体文件名规则仍为`聊天开始年月-聊天结束年月`

#   Step3 增加对引用消息类型的支持

在处理文本消息时，增加对这种数据结构的的检测。若以<?xml version="1.0"?>开头，且既存在<msg>标签，里面又有<appmsg>和<refermsg>标签，则不输出原始xml内容，而是主动进行提取。

消息格式为：

`{引用内容}\n-----\n{实际发送消息}`

引用内容为msg.appmsg.content标签内的内容

实际发送消息为msg.appmsg.title标签内的内容

-----

示例：对于这则消息，转化出的结果应该是

今天下午房间里一直没开灯 我也没睡着 一个人看动漫 感觉很孤单
...
<fromusr>wxid_1</fromusr>

<chatusr />

<displayname>hello</displayname>

<content>今天下午房间里一直没开灯 我也没睡着 一个人看动漫 感觉很孤单</content>

<msgsource>1</msgsource>

<createtime>170000000</createtime>

</refermsg>

</appmsg>

<fromusername>wxid_2</fromusername>
...
你好，我希望能快速找到数据库中有哪些聊天内容，请为我增加一个功能，每次运行时，都会导出一份json格式的通讯录记录列表，每个数据项包含以下字段：
-  id：微信id或者群组id
- 名称：微信昵称或群组名
- 类型：微信联系人、微信群、或者其他
- 总对话数据条数
